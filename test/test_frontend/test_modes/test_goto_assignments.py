...
def get_mode(editor):
    return frontend.get_mode(editor, pymodes.GoToAssignmentsMode)
...
def test_goto_variable(editor):
    ...
    mode = get_mode(editor)
    frontend.goto_line(editor, 2, len('print(a)') - 2)
    mode.request_goto()
    QTest.qWait(1000)
...
def test_goto_out_of_doc(editor):
    ...
    editor.setPlainText(code)
    mode = get_mode(editor)
    frontend.goto_line(editor, 2, len('print(a)') - 4)
...
def test_no_results(editor):
    ...
    frontend.goto_line(editor, 1, len('import foo') -2)
